      ...
      <!-- content column is here -->
        <div>
          ...
          <ul v-for="value in blok.list" class="list-disc ml-4 mt-4">
            <li class="text-base font-jakarta text-primary" >{{ value.text1 }} <span class="text-[#444850]">{{ value.text2 }}</span></li>
          </ul>
        </div>
        <!-- image column is here -->
        <div class=" lg:min-w-[425px]">
          <NuxtImg :src="blok.image.filename" class="w-full lg:h-[500px]"></NuxtImg>
        </div>
      </div>
    </section>
  </div>
